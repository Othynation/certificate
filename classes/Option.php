...
{
$sql = "UPDATE options SET option_value=:title,option_note=:link WHERE option_id=:id";
 ...
 public function update_general($web_title,$web_path,$web_url,$web_desc,$web_tags,$contact_email,$from_email,$logo,$fav,$currency,$reg,$id) 
     ...
     $web_path=$this->sanitize($web_path,'string');
               ...
               $web_tags=$this->sanitize($web_tags,'string');
                 ...
                 $fav=$this->sanitize($fav,'string');
                   $currency=$this->sanitize($currency,'string');
                             if($logo!= "")
                             {                              
              $sql= "UPDATE general SET logo= :logo WHERE id=:id"; 
              $stmt = $this->dbConnection->prepare($sql);
            ...
            $stmt->execute();
                             ...
                             if($fav!= "")
                             {                              
              $sql= "UPDATE general SET fav= :fav WHERE id=:id"; 
              $stmt = $this->dbConnection->prepare($sql);
            ...
            $stmt->execute();
                              ...
                              $sql= "UPDATE general SET web_title=:web_title,web_path=:web_path,web_url=:web_url,web_desc= :web_desc,web_tags=:web_tags,contact_email= :contact_email,from_email=:from_email,currency=:currency,reg=:reg WHERE id=:id"; 
              $stmt = $this->dbConnection->prepare($sql);
...
public function update_general_color($color,$id)
{
     $color=$this->sanitize($color,'string');                      
                              $sql= "UPDATE general SET color=:color WHERE id=:id"; 
              $stmt = $this->dbConnection->prepare($sql);
...
public function ajax_join($column,$table,$id,$search,$length,$start,$orderby,$type,$query)
    { 
$query =$query;
if(isset($_POST["is_type"]))
{
