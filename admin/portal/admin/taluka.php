...
{
  $result=$getCer->insert_taluka($sname,$sid);
}

           if($result)
    {   
   header("Location:taluka.php?action=Created");
    }
    else{
...
<select name="sid" class="form-control" required>
 ...
 <?php 
 $rows=$getCredit->fetch_all('pdistrict','dname','ASC');
 foreach($rows as $row)
 {
...
<div class="form-group">
  ...
  <div class="row">
            ...
            $id=$_GET['id'];
              ...
              if($st_name ==''){
            $error[] = 'Please enter the taluka name.';
        }
        $rows=$getCredit->get_by_string('ptaluka','tid',$id);
foreach($rows as $row)
{
    $oldst_name=$row['tname'];
}
if($oldst_name!=$st_name){
        $count=$getCredit->count_by_id('ptaluka','tname',$st_name); 
if($count>0)
{
$error[] = 'This Taluka is already created..';
}
}
         if(!isset($error)){ 

$result=$getCer->update_taluka($st_name,$sid,$id);
           if($result)
    {   
...
$rows=$getCredit->get_by_id('ptaluka','tid',$id);
foreach($rows as $row)
{
    $st_name=$row['tname'];
     $sid=$row['did'];
...
<div class="form-group">
<select name="sid" class="form-control" required>
 <option value="<?php echo $sid;?>" style="background: #EAEAEA;"><?php echo $getCredit->get_dname($sid);?></option> 
 <?php 
 $rows=$getCredit->fetch_all('pdistrict','dname','ASC');
 foreach($rows as $row)
 {
