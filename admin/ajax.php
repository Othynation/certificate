...
{
	case 'centres':
	 $column = array("cent_id","centre_name","city");
$table='centres';$id='cent_id';$orderby='DESC';$type='cent_id';
...
{
 $sub_array = array();
  $sub_array[] = '#'.$row["cent_id"];
    $sub_array[] = $row["centre_name"];
    $sub_array[] = $row["city"];
 $sub_array[] ='<a  href=\'?detect=edit&id='.$row['cent_id'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['cent_id'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
 $data[] = $sub_array;
}
	break; 
  case 'expenses':
 ...
 $sub_array = array();
  $sub_array[] = '#'.$row["exid"];
    ...
    $sub_array[] = $row["eamount"].' '.$getCredit->get_option_value('currency');
        $sub_array[] = $row["enote"];
           $sub_array[] = $row["edate"];
                    $sub_array[] = $getCredit->jump_to("users?detect=edit&id=".$row['id']."",$row["username"]);
 $sub_array[] ='<a  href=\'?detect=edit&id='.$row['exid'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['exid'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
 $data[] = $sub_array;
}
  break;
  case 'sh':
...
foreach($rows as $row)
{
 ...
 $sub_array[] ='<a  href=\'?detect=edit&id='.$row['clsid'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['clsid'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
 $data[] = $sub_array;
}
  break; 

  case 'formations':
   $column = array("fid","formation_name");
$table='formation';$id='fid';$orderby='DESC';$type='fid';
$search=$_POST["search"]["value"];$length=$_POST["length"];$start=$_POST['start'];
$rows=$getOption->ajax($column,$table,$id,$search,$length,$start,$orderby,$type);
$number_filter_row= $rows[0]; $rows=$rows[1];
$data = array();
 ...
 $sub_array = array();
  $sub_array[] = '#'.$row["fid"];
    $sub_array[] = $row["formation_name"];
       ...
       $sub_array[] ='<a class="btn btn-secondary" href=\'?detect=inscription&id='.$row['fid'].'\'">List</a>';  
 $sub_array[] ='<a  href=\'?detect=edit&id='.$row['fid'].'\'"><div class="icon-pencil"></div></a>';
...
foreach($rows as $row)
{
 ...
 $data[] = $sub_array;
  ...
  case 'users':
   $column = array("id","username","email","post","ustatus");
$table='ts_gtw_users';$id='id';$orderby='ASC';$type='id';
$search=$_POST["search"]["value"];$length=$_POST["length"];$start=$_POST['start'];
$post='prof';
$query="SELECT * 
FROM ts_gtw_users
...
foreach($rows as $row)
{
 ...
 $sub_array[] ='<a  href=\'?detect=edit&id='.$row['id'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['id'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
 $data[] = $sub_array;
}
  break; 
  case 'option':
   $column = array("option_id","option_name","option_value","type","option_note");
$table='options';$id='option_id';$orderby='ASC';$type='type';
$search=$_POST["search"]["value"];$length=$_POST["length"];$start=$_POST['start'];
$rows=$getOption->ajax($column,$table,$id,$search,$length,$start,$orderby,$type);
$number_filter_row= $rows[0]; $rows=$rows[1];
$data = array();
foreach($rows as $row)
{
 ...
 $sub_array[] = '<a  href=\'options?detect=edit&id='.$row['option_id'].'\'"><div class="icon-pencil"></div></a>';
 $data[] = $sub_array;
}
  break; 
  case 'registrations':
...
foreach($rows as $row)
{
 ...
 $sub_array[] = $getDatabase->easy_date($row["modcreated"]);
  $sub_array[] ='<a  href=\'?detect=edit&id='.$row['mid'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['mid'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
...
foreach($rows as $row)
{
 ...
 $sub_array[] = $getDatabase->easy_date($row["ccreated"]);
  ...
  $sub_array[] ='<a  href=\'?detect=edit&id='.$row['cid'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['cid'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
...
foreach($rows as $row)
{
 $sub_array = array();
    ...
    $sub_array[] ='<a class="btn btn-success"  href=\'?detect=seances&id='.$row['gid'].'\'">Seances</a>';
     $sub_array[] =  $getCredit->jump_to("registrations?detect=edit&id=".$row['uid']."",$row["username"]);
           $sub_array[] = $getDatabase->easy_date($row["gdate"]);
           $sub_array[] ='<a class="btn btn-success"  href=\'?detect=genseances&id='.$row['gid'].'\'">Generate</a>';
...
foreach($rows as $row)
{
 $sub_array = array();
  ...
  $sub_array[] ='<a  href=\'?detect=edit&id='.$row['cm_id'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=delComment&inid='.$inid.'&id='.$row['cm_id'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
...
foreach($rows as $row)
{
 $sub_array = array();
   ...
   $sub_array[] = '<a href=\'?detect=delexam&id='.$row['eid'].'&sid='.$row['sid'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
...
$sub_array[] ='<a  href=\'?detect=view&id='.$row['inid'].'\'"><img src="assets/icon/avatar.png" height="30"></a>';
 $sub_array[] ='<a  href=\'?detect=edit&id='.$row['inid'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['inid'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
 $data[] = $sub_array;
}
  break;
  case 'payments':
...
LEFT JOIN inscription ON registrations.reg_id=inscription.reg_id
 WHERE inscription.inid='$inid' AND
";
}
else 
{
    ...
    $sub_array[] = $row["fname"].' '.$row["lname"];
     ...
     $sub_array[] = $row["centre_name"];
       $sub_array[] = $getCredit->status($row["ustatus"]);
          ...
          $sub_array[] = $getCredit->get_prof($row['uid']);
                 $sub_array[] = $getDatabase->easy_date($row["date"]);
 $sub_array[] ='<a  href=\'?detect=edit&id='.$row['id'].'\'"><div class="icon-pencil"></div></a>';
   $sub_array[] = '<a href=\'?detect=del&id='.$row['id'].'\' onClick=\'return confirm("Are you sure you want to delete?")\'"><div class="icon-bin" style="color:red;"></div></a>';
